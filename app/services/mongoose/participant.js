...
const activateParticipant = async (req) => {
  ...
  if (!check) throw new NotFoundError('Participants have not yet registered');

  if (check && check.otp !== otp) throw new BadRequestError('Wrong otp code');
...
const getOneEvent = async (req) => {
  ...
  if (!result) throw new NotFoundError(`There are no events with id :  ${id}`);
...
const checkoutOrder = async (req) => {
  ...
  if (!checkingEvent) {
    throw new NotFoundError(`There are no events with id : event `);
  }

  const checkingPayment = await Payments.findOne({ _id: payment });

  if (!checkingPayment) {
    throw new NotFoundError(`There are no payment with id : payment`);
  }
  ...
  await tickets.forEach((tic) => {
    checkingEvent.tickets.forEach((ticket) => {
      if (tic.ticketCategories.type === ticket.type) {
        console.log(ticket.stock)
        if (tic.sumTicket > ticket.stock) {
          throw new NotFoundError('Stock ticket not enough');
        } else {
  ...
  const dataInvoice = {
    ...
    orderAmount: result.totalPay,
    paymentMethod: checkingPayment.type,
  }
  await invoiceMail(personalDetail.email, checkingEvent.title, dataInvoice)
